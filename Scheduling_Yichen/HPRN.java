...
public class HPRN{
	...
	public HPRN(ArrayList<Process> p, ArrayList<Process> s, boolean v){
		...
		if(runningList.isEmpty()){
			Collections.sort(readyList, new Comparator<Process>(){
				...
				public int compare(Process one, Process two){
					float A1 = (cycle - one.getA())*1.0f/Math.max(1, one.getT());
					int arrivingOrder1 = allProcesses.indexOf(one);
					float A2 = (cycle - two.getA())*1.0f/Math.max(1, two.getT());
			...
			// if(cycle == 28){
			// 	System.out.println("show penalty");
			// 	System.out.println((cycle - p.getA())*1.0f/Math.max(1, p.getT()));
			// 	System.out.println((cycle - readyList.get(0).getA())*1.0f/Math.max(1, readyList.get(0).getT()));
			// }
